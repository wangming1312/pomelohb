...
var handleMessage = function(self, session, msg) {
  ...
  var newSession = session.toFrontendSession();
  if (!newSession.uid && msg.route && msg.route.indexOf("connector") == -1 && msg.route.indexOf("gateHandler") == -1) {
    logger.error('---session 丢失-0-', newSession, msg);
  }
  self.server.globalHandle(msg, newSession , function(err, resp, opts) {
    if (resp && !msg.id) {
    ...
    self.send(msg.id, msg.route, resp, [session.id], opts,
      function () {
        logger.debug('wangming [%s] handleMessage session id: %s, session uid: %s, route: %j ', self.app.serverId, session.id, session.uid, msg.route, new Date().getTime());
      });
  });
};
