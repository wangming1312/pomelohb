...
pro.globalHandle = function (msg, session, cb) {
  if (!session.uid && msg.route && msg.route.indexOf("connector") == -1 && msg.route.indexOf("gateHandler") == -1) {
    logger.error('---session 丢失-1-', session,msg);
  }
  ...
  if (!session.uid && msg.route && msg.route.indexOf("connector") == -1 && msg.route.indexOf("gateHandler") == -1) {
    logger.error('---session 丢失-2-', session, msg);
  }
  ...
  if (!session.uid && msg.route && msg.route.indexOf("connector") == -1 && msg.route.indexOf("gateHandler") == -1) {
    logger.error('---session 丢失-3-', session, msg);
  }
  var self = this;
  var dispatch = function(err, resp, opts) {
    ...
    if (self.app.getServerType() !== routeRecord.serverType) {
      if (!session.uid && msg.route && msg.route.indexOf("connector") == -1 && msg.route.indexOf("gateHandler") == -1) {
        logger.error('---session 丢失-5-', session, msg);
      }
      ...
      if (!session.uid && msg.route && msg.route.indexOf("connector") == -1 && msg.route.indexOf("gateHandler") == -1) {
        logger.error('---session 丢失-6-', session, msg);
      }
...
var beforeFilter = function(isGlobal, server, msg, session, cb) {
  ...
  if (!session.uid && msg.route && msg.route.indexOf("connector") == -1 && msg.route.indexOf("gateHandler") == -1) {
    logger.error('---session 丢失-4-', session, msg);
  }
  if(fm) {
...
var doForward = function(app, msg, session, routeRecord, cb) {
  ...
  try {
    if (!session.uid && msg.route && msg.route.indexOf("connector") == -1 && msg.route.indexOf("gateHandler") == -1) {
      logger.error('---session 丢失-7-', session, msg);
    }
...
var doHandle = function(server, msg, session, routeRecord, cb) {
  ...
  var handle = function(err, resp, opts) {
    ...
    if (!session.uid && msg.route && msg.route.indexOf("connector") == -1 && msg.route.indexOf("gateHandler") == -1) {
      logger.error('---session 丢失-8-', session);
    }
